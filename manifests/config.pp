# @summary Defines what is in the sshd_config
#
# @example Basic Usage
#   class { 'ssh::config':
...
#
#      ExposeAuthInfo
#              Writes a temporary file containing a list of authentication methods and public credentials (e.g. keys) used to authenticate the user.  The location of the file
#              is exposed to the user session through the SSH_USER_AUTH environment variable.  The default is no.
#
#      FingerprintHash
...
#              specified methods (including wildcards) will be removed from the default set instead of replacing them.  The supported algorithms are:
#
#                    curve25519-sha256
...
#              ssh-keygen(1).
#
#      @param use_dns Specifies whether sshd(8) should look up the remote host name, and to check that the resolved host name for the remote IP address maps back to the very same IP
#              address.
#
